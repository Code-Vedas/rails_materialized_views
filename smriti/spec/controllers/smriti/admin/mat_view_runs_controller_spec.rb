...
RSpec.describe Smriti::Admin::MatViewRunsController, type: :controller do
  ...
  describe 'GET #index' do
    ...
    it 'authorizes access with correct arguments' do
      expect(controller).to have_received(:authorize_smriti!).with(:read, :smriti_runs)
    end
  ...
  describe 'GET #show' do
    ...
    before do
      allow(controller).to receive(:authorize_smriti!).and_return(true)
      allow(Smriti::MatViewRun).to receive(:find).with(run_id).and_return(run_obj)
      allow(controller).to receive(:render)
    ...
    it 'finds the run and authorizes it' do
      expect(Smriti::MatViewRun).to have_received(:find).with(run_id)
      expect(controller).to have_received(:authorize_smriti!).with(:read, :smriti_run, run_obj)
    end
