...
RSpec.describe Smriti::Admin::MatViewDefinitionsController, type: :controller do
  ...
  let(:config_double) do
    instance_double(Smriti::Configuration,
                    job_queue: 'default',
                    admin_ui: { row_count_strategy: :none })
  end

  before do
    allow(Smriti).to receive(:configuration).and_return(config_double)
  ...
  describe 'GET #index' do
    ...
    let(:service_response_one) { instance_double(Smriti::ServiceResponse, response: { exists: true }) }
    let(:service_response_two) { instance_double(Smriti::ServiceResponse, response: { exists: false }) }
    let(:checker_one) { instance_double(Smriti::Services::CheckMatviewExists, call: service_response_one) }
    let(:checker_two) { instance_double(Smriti::Services::CheckMatviewExists, call: service_response_two) }

    before do
      allow(Smriti::Services::CheckMatviewExists).to receive(:new).with(defn_one).and_return(checker_one)
      allow(Smriti::Services::CheckMatviewExists).to receive(:new).with(defn_two).and_return(checker_two)
      allow(controller).to receive(:render)
      get :index, params: { lang:, frame_id: }
    end

    it 'authorizes and assigns @definitions and @mv_exists_map' do
      expect(controller).to have_received(:authorize_smriti!).with(:read, :smriti_definitions)
  ...
  describe 'GET #show' do
    ...
    let(:service_response) { instance_double(Smriti::ServiceResponse, response: { exists: true }) }
    let(:checker) { instance_double(Smriti::Services::CheckMatviewExists, call: service_response) }
    ...
    before do
      allow(Smriti::MatViewDefinition).to receive(:find).with(defn.id.to_s).and_return(defn)
      ...
      allow(Smriti::Services::CheckMatviewExists).to receive(:new).with(defn).and_return(checker)
      allow(controller).to receive(:render)
      get :show, params: { lang:, frame_id:, id: defn.id }
    end

    it 'authorizes, assigns @definition, @mv_exists, @runs and renders' do
      expect(controller).to have_received(:authorize_smriti!).with(:read, :smriti_definition, defn)
      expect(controller.instance_variable_get(:@mv_exists)).to be(true)
      expect(controller.instance_variable_get(:@runs)).to eq(runs)
      expect(controller).to have_received(:render).with('show', formats: :html, layout: 'smriti/turbo_frame')
      expect(response).to be_successful
    end
  end
  ...
  describe 'GET #new' do
    ...
    before do
      allow(Smriti::MatViewDefinition).to receive(:new).and_return(defn)
    ...
    it 'authorizes, builds a new definition and renders' do
      expect(controller).to have_received(:authorize_smriti!).with(:create, :smriti_definition)
      expect(controller.instance_variable_get(:@definition)).to eq(defn)
      expect(controller).to have_received(:render)
        .with('form', hash_including(formats: :html, layout: 'smriti/turbo_frame'))
  ...
  describe 'GET #edit' do
    ...
    it 'authorizes and renders form' do
      expect(controller).to have_received(:authorize_smriti!).with(:update, :smriti_definition, defn)
      expect(controller.instance_variable_get(:@definition)).to eq(defn)
      expect(controller).to have_received(:render).with('form', formats: :html, layout: 'smriti/turbo_frame')
      expect(response).to be_successful
    end
  end
  ...
  describe 'POST #create' do
    ...
    context 'when save succeeds' do
      before do
        allow(Smriti::MatViewDefinition).to receive(:new)
    ...
    context 'when save fails' do
      ...
      it 'renders form with unprocessable status' do
        expect(controller).to have_received(:render).with(
          'form', formats: :html, layout: 'smriti/turbo_frame', status: :unprocessable_content
        )
      end
    end
  ...
  describe 'PATCH #update' do
    ...
    before do
      allow(Smriti::MatViewDefinition).to receive(:find).and_return(defn)
    end
  ...
  describe 'DELETE #destroy' do
    ...
    context "when frame_id != 'dash-definitions' renders empty with 298" do
      ...
      it 'renders the empty turbo frame with status 298' do
        expect(controller).to have_received(:render).with(
          'empty', formats: :html, layout: 'smriti/turbo_frame', status: 298
        )
      end
    end
  end
  ...
  describe 'POST #create_now' do
    ...
    it 'enqueues CreateViewJob with queue and args, then redirects (303)' do
      ...
      expect(controller).to have_received(:authorize_smriti!).with(:create, :smriti_definition_view, defn)
      expect(Smriti::Jobs::Adapter).to have_received(:enqueue).with(
  ...
  describe 'POST #refresh' do
    ...
    it 'enqueues RefreshViewJob and redirects (303)' do
      ...
      expect(controller).to have_received(:authorize_smriti!).with(:update, :smriti_definition_view, defn)
      expect(Smriti::Jobs::Adapter).to have_received(:enqueue).with(
  ...
  describe 'POST #delete_now' do
    ...
    it 'enqueues DeleteViewJob with cascade flag and redirects (303)' do
      ...
      expect(controller).to have_received(:authorize_smriti!).with(:destroy, :smriti_definition_view, defn)
      expect(Smriti::Jobs::Adapter).to have_received(:enqueue).with(
  ...
  describe '#build_matview_exists_map' do
    context 'when definitions are present' do
      ...
      let(:service_response_a) { instance_double(Smriti::ServiceResponse, response: { exists: true }) }
      let(:service_response_b) { instance_double(Smriti::ServiceResponse, response: { exists: false }) }
      let(:checker_a) { instance_double(Smriti::Services::CheckMatviewExists, call: service_response_a) }
      let(:checker_b) { instance_double(Smriti::Services::CheckMatviewExists, call: service_response_b) }

      it 'returns a map of definition names to existence boolean' do
        allow(Smriti::Services::CheckMatviewExists).to receive(:new).with(defn_a).and_return(checker_a)
        allow(Smriti::Services::CheckMatviewExists).to receive(:new).with(defn_b).and_return(checker_b)
