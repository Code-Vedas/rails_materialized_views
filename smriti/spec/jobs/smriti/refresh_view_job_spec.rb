...
RSpec.describe Smriti::RefreshViewJob, type: :job do
  before do
    ActiveJob::Base.queue_adapter = :test

    Smriti.configure do |c|
      c.job_queue = :smriti_test
    end
  end
    ...
    describe '#perform' do
      context 'when the service returns success' do
        it 'records a successful refresh run with core fields set' do
          ...
          run    = Smriti::MatViewRun.refresh_runs.order(created_at: :desc).first
      ...
      context 'when run fails to save' do
        it 'raises and does not attempt to update the run' do
          ...
          allow(Smriti::MatViewRun).to receive(:create!).and_raise(ActiveRecord::RecordInvalid)

          expect do
            perform_now_and_return(definition.id)
          end.to raise_error(Minitest::UnexpectedError)

          expect(Smriti::MatViewRun).to have_received(:create!).once
        end
      end
  ...
  context 'when the definition uses regular refresh' do
    ...
    let(:svc_class) { Smriti::Services::RegularRefresh }
  ...
  context 'when the definition uses concurrent refresh' do
    ...
    let(:svc_class) { Smriti::Services::ConcurrentRefresh }
  ...
  context 'when the definition uses swap refresh' do
    ...
    let(:svc_class) { Smriti::Services::SwapRefresh }
