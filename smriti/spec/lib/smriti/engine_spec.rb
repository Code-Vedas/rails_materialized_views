...
RSpec.describe Smriti::Engine, type: :engine do
  it 'inherits from Rails::Engine' do
    expect(described_class < Rails::Engine).to be(true)
  end

  it 'isolates the Smriti namespace' do
  ...
  describe 'project metadata helpers' do
    ...
    let(:fake_spec) do
      instance_double(
        Gem::Specification,
        name: 'smriti',
        homepage: 'https://example.com/smriti',
    ...
    context 'when a loaded spec is present' do
      ...
      it { expect(described_class.project_name).to eq('smriti') }
      it { expect(described_class.project_homepage).to eq('https://example.com/smriti') }
      ...
      it { expect(described_class.rubygems_uri).to eq('https://rubygems.org/gems/smriti') }
    end
    ...
    it 'returns Smriti::VERSION for project_version' do
      expect(described_class.project_version).to eq(Smriti::VERSION)
    end
  end

  describe 'smriti.importmap' do
    let(:engine) { described_class.instance }
    let(:engine_initializer) { engine.initializers.find { |i| i.name == 'smriti.importmap' } }
    ...
    around do |ex|
      prev = Smriti.importmap
      Smriti.importmap = nil
      ex.run
      Smriti.importmap = prev
    end
    ...
    context 'when Importmap is defined (then-branch)' do
      ...
      it 'builds the map, draws config, sets sweeper, installs before_action, and calls puts' do
        ...
        end.to change(Smriti, :importmap).from(nil)

        map = Smriti.importmap
    ...
    context 'when Importmap is NOT defined (else-branch via guard next)' do
      ...
      it 'is a no-op: does not set importmap, does not call on_load, does not puts' do
        expect do
          engine_initializer.run(instance_double(Rails::Application))
        end.not_to change(Smriti, :importmap)
