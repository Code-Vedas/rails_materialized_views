...
RSpec.describe Smriti::Services::CreateView do
  ...
  describe 'row count strategy handling' do
    ...
    context 'when row_count_strategy is :estimated' do
      ...
      it 'fetches estimated row counts' do
        ...
        expect(res.response[:row_count_before]).to eq(Smriti::Services::BaseService::UNKNOWN_ROW_COUNT) # unknown before creation
        expect(res.response[:row_count_after]).to eq(Smriti::Services::BaseService::UNKNOWN_ROW_COUNT) # analysis not run yet
        expect(mv_exists?(relname)).to be(true)
      end
    end
