...
RSpec.describe 'Dashboard', :feature do
  feature 'Dashboard UI' do
    ...
    scenario 'displays the header with logo and title' do
      expect(page).to have_link(href: '/smriti/en/admin', class: 'mv-brand', count: 1)

      within("a.mv-brand[href='/smriti/en/admin']") do
        expect(page).to have_css("img.mv-logo[alt='Smriti Admin']")
        expect(page).to have_css('span', text: 'Smriti Admin')
      end
    end
