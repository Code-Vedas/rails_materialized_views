...
RSpec.describe 'UI screenshots', :feature_app_screenshots do
  ...
  locales = if lang_code_env.nil? || lang_code_env.strip.downcase == 'all'
              Smriti::Engine.available_locales.map(&:to_s)
            else
              [lang_code_env.strip]
            end
  ...
  locales.each do |lang_code|
    ...
    lang_name = Smriti::Engine.locale_code_mapping[lang_code.to_sym]
    ...
    %w[light dark].freeze.each do |theme|
      it "captures screenshots for #{lang_name} (#{lang_code}) in #{theme} theme", :js do
        urls = [
          { name: 'Definitions List', url: "/smriti/#{lang_code}/admin" },
          { name: 'Definitions View',
            url: "/smriti/#{lang_code}/admin?open=definitions_view_#{example_definition.id}",
            wait_drawer_open: true },
          { name: 'Definitions New',
            url: "/smriti/#{lang_code}/admin?open=definitions_new",
            wait_drawer_open: true },
          { name: 'Definitions Edit',
            url: "/smriti/#{lang_code}/admin?open=definitions_edit_#{example_definition.id}",
            wait_drawer_open: true },
          { name: 'Runs List', url: "/smriti/#{lang_code}/admin?tab=runs" },
          { name: 'Runs View',
            url: "/smriti/#{lang_code}/admin?tab=runs&open=runs_view_#{example_run.id}",
            wait_drawer_open: true },
          { name: 'Preferences',
            url: "/smriti/#{lang_code}/admin?open=preferences_edit",
            wait_drawer_open: true }
        ]
