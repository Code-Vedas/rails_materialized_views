...
# Helpers for Turbo and related UI interactions in system tests (feature specs).
module TurboHelpers
  ...
  def wait_for_turbo_idle(timeout: Capybara.default_max_wait_time)
    ...
    expect(page).to have_no_css('body[busy]', wait: timeout)
  end
  ...
  def select_language(language, timeout: Capybara.default_max_wait_time)
    open_preferences
    within_drawer do
      select(language, from: 'locale')
      expect(page).to have_select('locale', selected: language)
      expect(page).to have_css('button[data-testid="preferences_save_button"]', wait: timeout)
      find('button[data-testid="preferences_save_button"]').click
    end
    wait_for_turbo_idle(timeout: timeout)
  end

  def select_theme(theme, timeout: Capybara.default_max_wait_time)
    open_preferences
    within_drawer do
      find("input[type='radio'][value='#{theme}']").click
      expect(page).to have_css('button[data-testid="preferences_save_button"]', wait: timeout)
      find('button[data-testid="preferences_save_button"]').click
    end
    wait_for_turbo_idle(timeout: timeout)
  ...
  def wait_drawer_open(timeout: Capybara.default_max_wait_time)
    wait_for_turbo_idle(timeout: timeout)
    expect(page).to have_css('div.mv-drawer-root.is-open', wait: timeout)
  end
