...
namespace :smriti do
  # --- SEED -----------------------------------------------------------
  desc 'Load demo datasets (usage: rake smriti:seed_demo[scale,--yes])'
  ...
  task :define_demo_views, [] => :environment do
    ...
    defs << Smriti::MatViewDefinition.where(name: 'mv_users').first_or_initialize.tap do |d|
      d.sql = <<~SQL.squish
    ...
    defs << Smriti::MatViewDefinition.where(name: 'mv_user_accounts').first_or_initialize.tap do |d|
      d.sql = <<~SQL.squish
    ...
    defs << Smriti::MatViewDefinition.where(name: 'mv_user_accounts_events').first_or_initialize.tap do |d|
      d.sql = <<~SQL.squish
    ...
    defs << Smriti::MatViewDefinition.where(name: 'mv_user_activity').first_or_initialize.tap do |d|
      d.sql = <<~SQL.squish
        SELECT u.id AS user_id,
               COUNT(a.*) AS accounts_count,
               COUNT(e.*) AS events_count,
    ...
    Rails.logger.info("[demo] Defined/updated #{defs.size} Smriti::MatViewDefinition records.")
  end
  ...
  task :bootstrap_demo, %i[scale yes] => :environment do |_t, args|
    Rake::Task['smriti:seed_demo'].invoke(args[:scale], args[:yes])
    Rake::Task['smriti:define_demo_views'].invoke

    # Create MVs via the gem’s tasks (skip confirms)
    Rake::Task['smriti:create_all'].reenable
    Rake::Task['smriti:create_all'].invoke(nil, '--yes')
    ...
    Rake::Task['smriti:refresh_all'].reenable
    Rake::Task['smriti:refresh_all'].invoke(nil, '--yes')
