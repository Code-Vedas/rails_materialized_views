...
# Usage:
#   bundle exec rake smriti:validate_demo[iterations]
# Examples:
#   bundle exec rake smriti:validate_demo[5]
#   bundle exec rake smriti:validate_demo            # defaults to 5
namespace :smriti do
  desc 'Benchmark baseline vs MV and write CSV (smriti:validate_demo[iterations])'
  task :validate_demo, [:iterations] => :environment do |_t, args|
    ...
    definitions = Smriti::MatViewDefinition.where(name: mv_names).order(:name).to_a
    ...
    CSV.open(out_csv, 'w') do |csv|
      ...
      definitions.each do |defn|
        ...
        begin
          result = SmritiDemo::Validator.new(defn, iterations: iterations).run
