...
Set a default queue for all Smriti jobs:
...
**Class:** `Smriti::CreateViewJob`
**Performs:** `Smriti::Services::CreateView`
...
- `concurrent` → `Smriti::Services::ConcurrentRefresh` (needs a **unique index**)
...
**Class:** `Smriti::DeleteViewJob`
**Performs:** `Smriti::Services::DeleteView`
...
# or directly
Smriti::DeleteViewJob.perform_later(defn.id, cascade: false)
```
...
- Use **Adapter** when:
  ...
  - You want to pass a **queue name** explicitly (from `Smriti.configuration.job_queue`).
...
```ruby
Smriti::MatViewDefinition.find_each do |defn|
  Smriti::Jobs::Adapter.enqueue(
    Smriti::RefreshViewJob,
    queue: Smriti.configuration.job_queue,
...
%w[mv_user_activity mv_user_accounts_events].each do |name|
  defn = Smriti::MatViewDefinition.find_by!(name: name)
...
```ruby
defn = Smriti::MatViewDefinition.find_by!(name: 'mv_stale_temp')
Smriti::DeleteViewJob.perform_later(defn.id, cascade: false)
```
...
- **Wrong queue**
  Verify `Smriti.configuration.job_queue` and any per-environment overrides.
