...
- **Create runs:** `Smriti::MatViewRun`, operation type `:create`
- **Refresh runs:** `Smriti::MatViewRun`, operation type `:refresh`
- **Delete runs:** `Smriti::MatViewRun`, operation type `:delete`
...
`Smriti::MatViewRun` has the following key attributes (columns):
...
> Services return a uniform `Smriti::ServiceResponse` that includes `status`, and `meta` (e.g., `sql`, strategies, options). Jobs persist the **meta** into the run row’s `meta`.
