...
if last_run&.status_success?
  puts "Last refresh OK in #{last_run.duration_ms} ms, rows=#{last_run.row_count}"
else
...
### Latest row_count (if tracked)

```ruby
MatViews::MatViewRun
  .select("DISTINCT ON (mat_view_definition_id) mat_view_definition_id, meta->>'row_count' AS row_count", created_at")
  .where(status: :success)
...
- **Dashboards:** chart `duration_ms` over time per view; show last status & row_count.
