...
- `smriti:create_by_name[view_name, force, row_count_strategy, --yes]`
- `smriti:create_by_id[mat_view_definition_id, force, row_count_strategy, --yes]`
- `smriti:create_all[force, row_count_strategy, --yes]`
...
- `smriti:refresh_by_name[view_name, row_count_strategy, --yes]`
- `smriti:refresh_by_id[mat_view_definition_id, row_count_strategy, --yes]`
- `smriti:refresh_all[row_count_strategy, --yes]`
...
- `smriti:delete_by_name[view_name, cascade, row_count_strategy, --yes]`
- `smriti:delete_by_id[mat_view_definition_id, cascade, row_count_strategy, --yes]`
- `smriti:delete_all[cascade, row_count_strategy, --yes]`
...
bin/rake 'smriti:create_by_name[mv_user_activity,--yes]'

# Create by id with force=true, skip confirm
bin/rake 'smriti:create_by_id[42,true,--yes]'

# Create all with force=false (default), skip confirm
bin/rake 'smriti:create_all[,--yes]'

# Using env overrides: YES=1, FORCE=true
YES=1 FORCE=true bin/rake 'smriti:create_by_name[mv_user_activity]'
```
...
bin/rake 'smriti:refresh_by_name[mv_user_activity,--yes]'

# Refresh by id with exact row count, skip confirm
bin/rake 'smriti:refresh_by_id[42,exact,--yes]'
...
YES=1 ROW_COUNT_STRATEGY=exact bin/rake 'smriti:refresh_by_name[mv_user_activity]'
```
...
bin/rake 'smriti:delete_by_name[mv_user_activity,--yes]'

# Delete by id with cascade=true, skip confirm
bin/rake 'smriti:delete_by_id[42,true,--yes]'

# Delete all with cascade=false (default), skip confirm
bin/rake 'smriti:delete_all[,--yes]'

# Using env override for cascade
YES=1 CASCADE=true bin/rake 'smriti:delete_by_name[mv_user_activity]'
```
...
YES=1 bin/rake 'smriti:refresh_by_name[public.mv_user_activity]'
```
...
- Create → `Smriti::CreateViewJob` with args `[mat_view_definition_id, force]`
- Refresh → `Smriti::RefreshViewJob` with args `[mat_view_definition_id, row_count_strategy]`
- Delete → `Smriti::DeleteViewJob` with args `[mat_view_definition_id, cascade]`

Queue is taken from `Smriti.configuration.job_queue` (default `:default` unless you changed it).
...
- Tasks use `Rails.logger.info` (no `puts`), e.g.:
  ...
  ```bash
  [smriti] Enqueued RefreshViewJob for definition #42 (mv_user_activity), row_count_strategy=estimated
  ```

- If no definitions are found (e.g., `*_all`), they log:

  ```bash
  [smriti] No mat view definitions found.
  ```
...
- **`No Smriti::MatViewDefinition found for "foo"`** - unknown definition.
- **`Materialized view public.foo exists, but no Smriti::MatViewDefinition record was found for name="foo"`** - schema-qualified MV exists but no definition; intentional guardrail.
...
The tasks are implemented in `lib/tasks/smriti_tasks.rake` and rely on `Smriti::Tasks::Helpers`:
...
- Project policies: [SECURITY](https://github.com/Code-Vedas/smriti/blob/main/SECURITY.md) · [CONTRIBUTING](https://github.com/Code-Vedas/smriti/blob/main/CONTRIBUTING.md) · [CODE_OF_CONDUCT](https://github.com/Code-Vedas/smriti/blob/main/CODE_OF_CONDUCT.md)
