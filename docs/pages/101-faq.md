...
Practical answers to common questions and failure modes when working with **smriti** (definitions → services → jobs → rake tasks), the **demo** app, and PostgreSQL materialized views.
...
- It **does not guess.** The adapter calls the unified API:
  `Smriti::Jobs::Adapter.enqueue(job, queue:, args:)`
...
- Rake task: `smriti:delete_by_name[mv_name,true,--yes]`
...
- Pass `--yes` as the last arg **or** set `YES=1`:
  ...
  YES=1 bin/rake 'smriti:refresh_all[]'
  ```

### “Which arguments do tasks accept?”

- **Create**: `[view_name, force, --yes]` / `[mat_view_definition_id, force, --yes]` / `[force, --yes]`
...
- `Smriti.configuration.job_queue` (default `:default`). Override in your initializer.
...
- Create views first (bootstrap demo or your own definitions):
  ...
  # or
  bin/rake 'smriti:create_all[,--yes]' && bin/rake 'smriti:refresh_all[,--yes]'
  ```

### “Found MVs in DB but no definitions”

- The validator needs **definitions** to get the **baseline SQL**.
- Ensure `Smriti::MatViewDefinition` exists for each MV name.
...
- Set up a spy:
  ...
  expect(Smriti::Jobs::Adapter).to have_received(:enqueue)
    .with(Smriti::RefreshViewJob, queue: anything, args: [defn.id, :estimated])
  ```
...
| `No Smriti::MatViewDefinition found`                     | Name mismatch / missing definition          | Create the definition or rename MV              |
