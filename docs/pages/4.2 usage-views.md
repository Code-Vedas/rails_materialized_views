...
```ruby
# Direct service call
result = Smriti::Services::CreateView.call(definition.id, force: true)
if result.success?
...
```bash
# Create by name
bundle exec rake smriti:create_by_name\[my_mat_view,force,--yes]
# Create by id
bundle exec rake smriti:create_by_id\[1,force,--yes]
# Create all
bundle exec rake smriti:create_all\[force,--yes]
```
...
```ruby
# Direct service call
result = Smriti::Services::RefreshView.call(definition.id, :estimated)
if result.success?
...
```bash
# Refresh by name
bundle exec rake smriti:refresh_by_name\[my_mat_view,estimated,--yes]
# Refresh by id
bundle exec rake smriti:refresh_by_id\[1,exact,--yes]
# Refresh all
...
```ruby
# Direct service call
result = Smriti::Services::DeleteView.call(definition.id, cascade: true)
if result.success?
...
```bash
# Delete by name
bundle exec rake smriti:delete_by_name\[my_mat_view,cascade,row_count_strategy,--yes]
# Delete by id
bundle exec rake smriti:delete_by_id\[1,cascade,row_count_strategy,--yes]
# Delete all
