...
- what to **name** the materialised view,
...
Once a definition exists, you can create, refresh (regular/concurrent/swap), and delete the materialised view using services, jobs, or rake tasks.
...
- **Concurrent refresh without unique index**
  You’ll get `ERROR: cannot refresh materialised view concurrently without a unique index`. Design your result set to include a unique key, then index it.
