...
1. [Create an issue](https://github.com/Code-Vedas/smriti/issues/new/choose) in the repository.
...
Seed data for dummy app tables (`users`, `accounts`, `events`, `sessions`) is provided via `smriti/spec/dummy/db/seeds.rb` file. Run `rails db:seed` to populate the database with this data when you are in `smriti` directory.
...
The smoke specs verify that `Smriti::Jobs::Adapter.enqueue` dispatches correctly to each adapter.
...
To release a new version of the `smriti` engine, follow these steps.

Pick the version number from draft release notes on the
[releases page](https://github.com/Code-Vedas/smriti/releases).
...
2. **Update the version**
   Update the version in `smriti.gemspec` to `<version>`.
...
4. **Docs sweep**
   ...
   - Root `README.md` and `smriti/README.md`
   - `smriti_demo/README.md` (new commands/configs if any)
...
11. **Create a GitHub Release**
    ...
    [GitHub releases page](https://github.com/Code-Vedas/smriti/releases).
